...
import (
	...
	"github.com/WlinkNET/xpense_chain/gossip/protocols/dag/dagstream"
...
func (p *peer) SendPeerInfoRequest() error {
	...
	if !p.Peer.RunningCap(ProtocolName, []uint{_Xpense_64, _Xpense_65}) {
		return nil
	}
...
func (p *peer) SendEndPointUpdateRequest() error {
	...
	// sending the request. This request would lead to a disconnect
	// if the peer doesn't understand it.
	if !p.Peer.RunningCap(ProtocolName, []uint{_Xpense_65}) {
		return nil
	}
